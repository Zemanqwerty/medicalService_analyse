...
import addFile from '../../sources/images/addFile.png'
    ...
    const sendFile = () => {
        ...
        const uploadResponse = UploadService.upload(firstName, lastName, report, age, sex, files).then((response) => {
            ...
            if(response?.status == 200){
                ...
                store.setResponse(response.data.message);
                setFirstName('');
                setLastName('');
                setReport('');
                setAge(null);
    ...
    const handleFilesChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files) {
            if (files.length >= 12) {
                ...
                store.setResponse('Выбрано максимальное количество файлов');
                return
            }
                ...
                <div className={styles.sendFileFormWrapper}>
                    ...
                    <div>
                        <input
                            ...
                            multiple
                        />
                        <button className={styles.addFilesBtn} onClick={handleButtonClick}>
                    ...
                    <ul className={styles.filesList}>
                        ...
                        <li key={index} className={styles.liFile}><div className={styles.liMarker}></div> {file.name}</li>
                        ))}
                    </ul>
