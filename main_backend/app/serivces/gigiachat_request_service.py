...
def api_text_generation(analyse_text: str, reference_text):
    ...
    "messages": [
        {
        "role": "user",
        "content": f'От лица специалиста, расскажи о результатах анализа из первого текста - {analyse_text} исходя из информации о значениях во втором - {reference_text}. расшифруй результаты таким образом, чтобы любой человек понял. В своём ответе не указывай, что ты опирался на какой-либо текст, а просто дай своё заключение'
        }
    ],
    "temperature": 0.5,
    "top_p": 0.1,
